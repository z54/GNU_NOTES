...
> sbin，boot，dev，etc，home，init，lib，lib32，lib64，media，mnt，opt，proc，root，run，sbin，srv，sys，tmp，usr，var
...
4. /etc/fstab
    ...
    指定启动需要自动安装的文件系统列表。也包括用swapon -a启用的s w a p区的信息。
...
10. /etc/mtab
    ...
    前安装的文件系统列表。由脚本(scritp)初始化，并由mount 命令自动更新。当需要一个当前安装的文件系统的列表时使用(例如df 命令)。
